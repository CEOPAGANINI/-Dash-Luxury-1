  ...
  return (
    <button
      ...
      className={
        compact
          ? styles.iconButton
          : "dash-sidebar-block dash-tema-botao focus-visible:ring-ring flex min-h-10 w-full items-center gap-3 px-3 text-left text-sm font-semibold outline-none focus-visible:ring-2"
      }
      aria-pressed={branco}
      aria-label={
        branco
    ...
    >
      <SolarIcon name={branco ? "sun" : "moon"} className="size-4 shrink-0" />
      <span className={compact ? "sr-only" : "truncate"}>
        {montado ? (branco ? "Tema branco" : "Tema preto") : "Tema"}
      </span>
    </button>
  );
}
